...
<div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-black p-4">
  <div className="flex flex-col sm:flex-row gap-4 mb-6 items-center">
    ...
    <Link 
      to="/add-product" 
      className="bg-purple-700 w-full text-center sm:w-40 text-white font-semibold py-2 rounded-md hover:bg-purple-800 transition duration-300"
    >
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {filteredProducts.map((item) => (
      <div 
        key={item._id} 
        className="bg-white backdrop-blur-sm rounded-xl shadow-lg overflow-hidden border border-purple-200/30 hover:shadow-xl transition-shadow"
      >
        ...
        <div className="p-5">
          ...
          <p className="text-purple-700 mb-1">Price: {item.price}</p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link 
              ...
              className="bg-purple-700 hover:bg-purple-600 text-white py-1 px-3 rounded flex-1 text-center transition-colors text-sm"
            >
              <button>Update</button>
            </Link>
            <Link 
              to={`/product-detail/${item._id}`}
              className="border border-purple-600 text-purple-600 hover:bg-purple-50 py-1 px-3 rounded flex-1 text-center transition-colors text-sm"
            >
              <button>View</button>
            </Link>
          </div>
